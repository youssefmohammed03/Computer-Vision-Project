...
def process_barcode_image(image_path):
    ...
    if x_min < x_max and y_min < y_max:
        ...
        cv2.imwrite('working_on.jpg', cropped_image)
...
def crop_white_space(image):
    ...
    while np.sum(image[:, 0] < 250) < 20:  # Check if the first column is entirely white
        image = image[:, 1:]
    while np.sum(image[:, -1] < 250) < 20:  # Check if the last column is entirely white
        image = image[:, :-1]
    
    while np.sum(image[0, :] < 250) < 20:
        image = image[1:, :]
    while np.sum(image[-1, :] < 250)< 180:
        image = image[:-1, :]
